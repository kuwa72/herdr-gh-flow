...
- **左ペイン（人間の司令塔）**:
  - `hgf` 実行、`git status` / `git diff` によるリアルタイムなコード改変監視、手動テスト検証、`bin/ci-wait` によるCI追従。
- **右ペイン（エージェント作業場）**:
...
- **単体 CLI バイナリ `hgf`**: Go 製の単一静的バイナリ。`fzf` などの外部依存を組み込み TUI で排除する。
- **`gh extension` 互換**: 拡張名 `hgf`、コマンド `gh hgf` として利用可能。リリース資産は `gh-hgf-<os>-<arch>` の命名規則を満たす。
- **Herdr 環境自動検出**: `HERDR_ENV=1` および `herdr` コマンドの有無を判定し、Herdr がある場合は右ペインを生成、ない場合はインラインフォールバックする。

### 6.2 基本UXモデル

- **1 ストローク選定**: 組み込み TUI で Issue 選択とエージェント決定を同時に行う。
